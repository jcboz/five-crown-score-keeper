  ...
  const onGestureEvent = useAnimatedGestureHandler({
    ...
    onEnd: ({ velocityX, velocityY }) => {
      ...
      if (
        ...
        // console.log('index', tplayers[playerTurnCounter].subHand.indexOf(children.props.item));
        let inDeck = false;
        for (let j = 0; j < tplayers[playerTurnCounter].subHand.length; j++) {
          for (let i = 0; i < tplayers[playerTurnCounter].subHand[j].length; i++) {
            if (
              tplayers[playerTurnCounter].subHand[j][i].suite === children.props.item.suite &&
              tplayers[playerTurnCounter].subHand[j][i].value === children.props.item.value &&
              tplayers[playerTurnCounter].subHand[j][i].cardID === children.props.item.cardID
            ) {
              inDeck = true;
            }
          }
        ...
        if (!inDeck) {
          tplayers[playerTurnCounter].subHand[2].push(children.props.item);
        }
        console.log('temp player! ', tplayers[playerTurnCounter].subHand);
        runOnJS(setPlayers)(tplayers);
        console.log('player', players[playerTurnCounter].subHand); // children.props.item is what we will want to push to a players subhand
      }
      // else if (
      //   between(translation.x.value, offset.x.value, offset.x.value + offset.width.value) &&
      //   between(translation.y.value, offset.y.value - CARD_HEIGHT, offset.y.value)
      ...
      //   // console.log('card: ', children.props.item);
      //   // console.log('subhand', tplayers[playerTurnCounter].subHand[1]);
      //   // console.log('index', tplayers[playerTurnCounter].subHand.indexOf(children.props.item));
      //   let inDeck = false;
      //   for (let j = 0; j < tplayers[playerTurnCounter].subHand.length; j++) {
      //     for (let i = 0; i < tplayers[playerTurnCounter].subHand[j].length; i++) {
        ...
        for (let j = 0; j < tplayers[playerTurnCounter].subHand.length; j++) {
          for (let i = 0; i < tplayers[playerTurnCounter].subHand[j].length; i++) {
            if (
              ...
              tplayers[playerTurnCounter].subHand[j].splice(children.props.item, 1);
            }
          }
        }
        console.log('temp player! ', tplayers[playerTurnCounter].subHand);
